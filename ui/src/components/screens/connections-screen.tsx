...
import { api } from "../../api";
import { Plus, Search, Link2, AlertTriangle, ChevronDown } from "lucide-react";
...
export default function ConnectionsScreen() {
  ...
  const connectionStats = useMemo(() => {
    ...
    const needsAttention = conns.filter((c: any) => {
      ...
      const rl =
        c.rateLimitedUntil && new Date(c.rateLimitedUntil) > new Date();
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Manage your AI provider accounts. Connect Kiro, OpenAI or any
      ...
      {conns.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <Card
            className={
              connectionStats.needsAttention > 0 ? "border-destructive/40" : ""
            }
          >
            <CardContent className="p-4">
              <div
      ...
      {conns.length === 0 ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-lg border py-12 text-center">
          <p className="text-sm text-muted-foreground">
            No connections matching "{searchQuery.trim()}"
          </p>
          <Button
            variant="link"
        ...
        <div className="space-y-6">
          {groupedConns.map((group) => {
            ...
            return (
              <div key={group.id}>
                <div
                  className="flex items-center gap-2 mb-3 cursor-pointer select-none group/col"
                  onClick={() => toggleGroup(group.id)}
                >
                  <div
                    className={cn(
                      "flex h-7 w-7 items-center justify-center rounded-lg border transition-transform group-hover/col:scale-105",
                      group.colorClass,
                    )}
                  >
                    {group.icon}
