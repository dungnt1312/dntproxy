...
export default function ConnectionCard({
    ...
    const renderBadgeStatus = () => {
        if (!c.isActive) return <Badge variant="outline" className="bg-muted text-muted-foreground border-border">Idle</Badge>;
        if (isRL) return <Badge variant="outline" className="bg-amber-500/10 text-amber-600 dark:text-amber-400 border-amber-500/20">Rate Limited</Badge>;
        if (hasIssue) return <Badge variant="outline" className="bg-destructive/10 text-destructive dark:text-red-400 border-destructive/20 cursor-pointer hover:bg-destructive/20" onClick={() => setIsLogOpen(true)}>Error</Badge>;
        ...
        <>
            <Card
                className={cn(
                    ...
                    hasIssue && c.isActive && 'border-destructive/30 shadow-destructive/10',
                )}
            >
                    ...
                    <div className="flex flex-col gap-2">
                        <div className="flex items-center gap-1.5 text-[11px] text-muted-foreground pl-0.5">
                            ...
                            <span 
                                ...
                                onClick={(e) => { e.stopPropagation(); onEditModels(c); }}
                                title="Configure available models"
                            >
                                {c.supportedModels?.length ? `${c.supportedModels.length} models` : 'All models'}
                            </span>
                        </div>
                        ...
                        <div className="rounded-md bg-muted/40 p-2.5  flex flex-col justify-center w-full">
                            {!c.isActive ? (
                                <p className="text-[10px] text-muted-foreground italic">Quota check unavailable when inactive.</p>
                            ) : quotaResult || quotaLoading ? (
                                <QuotaPanel data={quotaResult} loading={quotaLoading} onRefresh={handleCheckQuota} />
                            ) : (
                                <div className="flex items-center justify-between">
                                    <span className="text-[10px] text-muted-foreground italic">Quota not loaded</span>
                                    <Button variant="secondary" size="sm" className="h-5 text-[10px] px-2 font-medium" onClick={handleCheckQuota}>
                                        Load
                                    </Button>
                                </div>
                            )}
                ...
                <CardFooter className="mt-auto shrink-0 px-3 py-2.5 border-t border-border/30 bg-transparent flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2 min-w-0">
                        ...
                        >
                            {testResult?.loading ? <Loader2 className="h-3 w-3 animate-spin mr-1.5" /> : <TestTube className="h-3 w-3 mr-1.5" />}
                            Test
                        </Button>

                        {testResult && !testResult.loading && (
                            <span className={cn('text-[10px] font-medium px-2 py-1 rounded-md truncate max-w-[80px]',
                                testResult.status === 'ok' ? 'text-emerald-600 bg-emerald-500/10' : 'text-destructive bg-destructive/10'
                            )}>
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                        ...
                        <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="w-48 shadow-lg">
                                {(c.authType === 'apikey' || c.hasApiKey) && onEditConnection && (
                                    <DropdownMenuItem onClick={() => onEditConnection(c)} className="gap-2 cursor-pointer text-xs py-2">
                                        <Edit3 size={15} /> Edit Connection
                                    </DropdownMenuItem>
                                )}
                                <DropdownMenuItem onClick={() => onEditModels(c)} className="gap-2 cursor-pointer text-xs py-2">
                                    <Settings2 size={15} /> Configure Models
                                </DropdownMenuItem>
                                <DropdownMenuItem onClick={() => setIsLogOpen(true)} className="gap-2 cursor-pointer text-xs py-2 text-foreground">
                                    <TerminalSquare size={15} /> View System Logs
                                </DropdownMenuItem>
                                {(isRL || c.backoffLevel > 0) && (
                                    <DropdownMenuItem onClick={handleResetCooldown} className="gap-2 cursor-pointer text-xs py-2 text-amber-600 focus:text-amber-700">
                                        <RefreshCw size={15} /> Reset Cooldown
                                    </DropdownMenuItem>
                                )}
                                <DropdownMenuSeparator />
                                <DropdownMenuItem onClick={() => onDelete(c.id, c.name)} className="gap-2 text-xs py-2 text-destructive focus:text-destructive focus:bg-destructive/10 cursor-pointer">
