...
// ExportConnections exports the listed connections by ID. An empty list is rejected.
func ExportConnections(store port.CredentialStore, connectionIDs []string) (*BackupData, error) {
	if len(connectionIDs) == 0 {
		return nil, fmt.Errorf("connectionIds required")
	}
	appCfg, err := store.Load()
	if err != nil {
		return nil, err
	...
	idMap := make(map[string]bool, len(connectionIDs))
