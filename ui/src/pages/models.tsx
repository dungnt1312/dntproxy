...
function ProviderIcon({ provider, size = 16 }: { provider: string; size?: number }) {
  if (provider === 'kiro') return <AwsLogo size={size} />
...
export default function Models() {
  ...
  const sections = [
    { key: 'kiro', label: 'Kiro AI', models: filterModels(kiroModels), icon: <AwsLogo size={18} />, colorClass: 'stat-card-green', accentColor: '#FF9900' },
    { key: 'openai', label: 'OpenAI', models: filterModels(openaiModels), icon: <OpenAILogo size={18} />, colorClass: 'stat-card-blue', accentColor: '#10a37f' },
    { key: 'combo', label: 'Combos', models: filterModels(comboModels), icon: <Layers size={18} className="text-[var(--purple)]" />, colorClass: 'stat-card-purple', accentColor: '#a855f7' },
    { key: 'other', label: 'Other', models: filterModels(otherModels), icon: <Box size={18} className="text-[var(--text-muted)]" />, colorClass: '', accentColor: '#64748b' },
  ].filter(s => s.models.length > 0)

  const filteredAliases = Object.entries(aliases).filter(([a, m]) =>
    !q || a.toLowerCase().includes(q) || m.toLowerCase().includes(q)
  )
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">
            All available models across your providers, combos, and aliases.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        <div className="glass stat-card stat-card-green cursor-default">
          <div className="text-[10px] uppercase font-bold tracking-wider text-[var(--text-dim)] mb-1 flex items-center gap-1.5">
            <AwsLogo size={12} /> Kiro
          </div>
          <div className="text-xl font-bold" style={{ fontFamily: 'var(--font-heading)' }}>{kiroModels.length}</div>
        </div>
        <div className="glass stat-card stat-card-blue cursor-default">
          <div className="text-[10px] uppercase font-bold tracking-wider text-[var(--text-dim)] mb-1 flex items-center gap-1.5">
            <OpenAILogo size={12} /> OpenAI
          </div>
          <div className="text-xl font-bold" style={{ fontFamily: 'var(--font-heading)' }}>{openaiModels.length}</div>
        </div>
        <div className="glass stat-card stat-card-purple cursor-default">
          <div className="text-[10px] uppercase font-bold tracking-wider text-[var(--text-dim)] mb-1 flex items-center gap-1.5">
            <Layers size={12} className="text-[var(--purple)]" /> Combos
          </div>
          <div className="text-xl font-bold" style={{ fontFamily: 'var(--font-heading)' }}>{comboModels.length}</div>
        </div>
        <div className="glass stat-card stat-card-amber cursor-default">
          <div className="text-[10px] uppercase font-bold tracking-wider text-[var(--text-dim)] mb-1 flex items-center gap-1.5">
            <ArrowRight size={12} className="text-[var(--warning)]" /> Aliases
          </div>
          <div className="text-xl font-bold" style={{ fontFamily: 'var(--font-heading)' }}>{Object.keys(aliases).length}</div>
        </div>
      </div>
      ...
      {sections.map(section => (
        <div key={section.key} className="mb-6">
          <div className="flex items-center gap-2 mb-3">
            <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ backgroundColor: `${section.accentColor}15`, border: `1px solid ${section.accentColor}25` }}>
              {section.icon}
            </div>
            <h3 className="text-sm font-semibold" style={{ fontFamily: 'var(--font-heading)' }}>{section.label}</h3>
          ...
          <div className="flex flex-wrap gap-2">
            {section.models.map((m: any) => (
              ...
              >
                ...
                <span className="font-mono text-xs text-[var(--text-muted)] group-hover:text-[var(--text)] transition-colors">{m.id}</span>
                {m.models?.length > 0 && (
      ...
      <div className="mt-8">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold" style={{ fontFamily: 'var(--font-heading)' }}>Aliases</h3>
          ...
          <div className="glass p-4 mb-4 animate-slide-up">
            <div className="flex gap-3 items-end flex-wrap">
              <div className="flex-1 min-w-[120px]">
                <label className="block text-xs text-[var(--text-muted)] mb-1.5 font-medium">Alias</label>
                <input value={alias} onChange={e => setAlias(e.target.value)} placeholder="sonnet" className="glass-input w-full" />
              </div>
              <div className="flex-1 min-w-[180px]">
                <label className="block text-xs text-[var(--text-muted)] mb-1.5 font-medium">Target Model</label>
                <input value={model} onChange={e => setModel(e.target.value)} placeholder="kiro/claude-sonnet-4.5" className="glass-input w-full" />
              </div>
              <div className="flex items-center gap-2">
                <button onClick={handleAddAlias} className="btn-primary py-[10px]">Save</button>
                <button onClick={() => { setShowAdd(false); setAlias(''); setModel(''); }} className="btn-icon">
                  <X size={16} />
                </button>
              </div>
            </div>
        ...
        {filteredAliases.length === 0 && !showAdd ? (
          ...
          <div className="space-y-2">
            {filteredAliases.map(([a, m]) => (
              <div key={a} className="glass-sm px-4 py-3 flex items-center justify-between hover:border-[var(--border-hover)] transition-all group">
