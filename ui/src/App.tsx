...
const navItems: NavItem[] = [
  ...
  { path: '/playground', label: 'Playground', icon: MessageSquare, group: 'Overview' },
  { path: '/connections', label: 'Connections', icon: Link2, group: 'Management' },
  { path: '/models', label: 'Models & Aliases', icon: Boxes, group: 'Management' },
  ...
  { path: '/backup', label: 'Backup', icon: HardDriveDownload, group: 'System' },
]
    ...
    <div className="min-h-screen bg-background md:flex">
      ...
      >
        <div className="flex items-center gap-3 px-4 h-16 border-b border-border shrink-0">
          ...
          {sidebarOpen && (
            <span className="font-semibold text-lg tracking-tight truncate">Dntproxy</span>
          )}
        </div>
        ...
        <ScrollArea className="flex-1 py-3">
          {Object.entries(groupedItems).map(([group, items]) => (
            <div key={group} className="mb-2">
              ...
              {items.map(item => <NavButton key={item.path} item={item} />)}
            </div>
          ))}
        </ScrollArea>
        ...
        <div className="border-t border-border p-2 shrink-0 flex items-center gap-1">
          <Tooltip delayDuration={0}>
            ...
            <TooltipContent side="right">{mounted && theme === 'dark' ? 'Light Mode' : 'Dark Mode'}</TooltipContent>
          </Tooltip>
        ...
        <SheetContent side="left" className="w-[280px] p-0 md:hidden">
          <SheetHeader className="border-b border-border px-4 py-3 text-left">
            <div className="flex items-center gap-3">
              ...
              <div>
                <SheetTitle className="text-base tracking-tight">Dntproxy</SheetTitle>
                <SheetDescription className="text-xs">Navigation</SheetDescription>
              </div>
            </div>
          </SheetHeader>
      ...
      >
        <header className="sticky top-0 z-30 flex items-center justify-between border-b border-border bg-background/95 px-4 py-3 backdrop-blur md:hidden">
          <div className="flex items-center gap-2 min-w-0">
            <Button variant="outline" size="icon" onClick={() => setMobileMenuOpen(true)} className="h-9 w-9 shrink-0">
              <Menu className="h-4 w-4" />
            </Button>
            <p className="truncate text-sm font-semibold">{currentNavItem?.label ?? 'Dntproxy'}</p>
          </div>
          <Button variant="ghost" size="icon" onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')} className="h-9 w-9 shrink-0">
            {mounted && theme === 'dark' ? <Sun className="w-4 h-4" /> : <Moon className="w-4 h-4" />}
          </Button>
        </header>

        <div className="max-w-[1600px] p-4 md:p-6">
          <Routes>
