...
async function goRequest<T = unknown>(path: string, options?: RequestInit): Promise<T> {
...
function mapConnection(go: any) {
  const status = go.testStatus === 'active'
    ? 'active'
    : go.rateLimitedUntil
      ? 'rate_limited'
      : go.lastError
...
export const goApi: any = {
  // Connections
  getConnections: () =>
    goRequest<any[]>('/connections').then((conns) => (Array.isArray(conns) ? conns : []).map(mapConnection)),

  createConnection: (data: { name: string; provider: string; authType: string; authConfig?: string }) => {
    ...
    if (data.provider === 'openai-compatible') {
      ...
      body.name = data.name
      return goRequest('/connections/add-custom', { method: 'POST', body: JSON.stringify(body) })
    }
  ...
  updateConnection: (id: string, data: Record<string, unknown>) =>
    goRequest(`/connections/${id}`, { method: 'PUT', body: JSON.stringify(data) }),
  ...
  testModel: (id: string, model: string) =>
    goRequest(`/connections/${id}/test-model`, { method: 'POST', body: JSON.stringify({ model }) }),

  // Combos
  getCombos: () =>
    goRequest<any[]>('/combos').then((combos) => (Array.isArray(combos) ? combos : []).map(mapCombo)),

  createCombo: (data: { name: string; models?: string[]; connectionIds?: string[] }) =>
    goRequest('/combos', { method: 'POST', body: JSON.stringify(toComboPayload(data as any)) }),

  updateCombo: (id: string, data: Record<string, unknown>) =>
    goRequest(`/combos/${id}`, { method: 'PUT', body: JSON.stringify(toComboPayload(data as any)) }),

  deleteCombo: (id: string) =>
    goRequest(`/combos/${id}`, { method: 'DELETE' }),

  // Aliases
  getAliases: () => goRequest<AliasMap>('/aliases').then((aliases) => aliases || {}),

  setAlias: (alias: string, model: string) =>
    goRequest('/aliases', { method: 'POST', body: JSON.stringify({ alias, model }) }),
  ...
  deleteKey: (id: string) =>
    goRequest(`/keys/${id}`, { method: 'DELETE' }),

  // Settings
  getSettings: () =>
    goRequest('/settings').then(mapSettings),
  ...
  updateSettings: (data: Record<string, unknown>) => {
    ...
    return goRequest('/settings', { method: 'PUT', body: JSON.stringify(payload) }).then(mapSettings)
  },
  ...
  getLogSummary: (filters?: Record<string, unknown>) => {
    ...
    return goRequest<any>(`/logs/summary${query}`).then((summary) => ({
      ...
      successRequests: Number(summary?.successRequests ?? ((summary?.requests ?? 0) - (summary?.errors ?? 0))),
      errorRequests: Number(summary?.errorRequests ?? summary?.errors ?? 0),
      avgLatencyMs: Number(summary?.avgLatencyMs ?? 0),
      requestsByHour: Array.isArray(summary?.requestsByHour) ? summary.requestsByHour : [],
      requestsByProvider: Array.isArray(summary?.requestsByProvider) ? summary.requestsByProvider : [],
      requestsByStatus: Array.isArray(summary?.requestsByStatus) ? summary.requestsByStatus : [],
      recentErrors: Array.isArray(summary?.recentErrors) ? summary.recentErrors : [],
    }))
  },
  ...
  startIDC: (data: { startUrl: string; region?: string }) =>
    goRequest('/auth/kiro/start-idc', { method: 'POST', body: JSON.stringify(data) }),

  pollAuth: (sessionId: string) =>
    goRequest('/auth/kiro/poll', { method: 'POST', body: JSON.stringify({ sessionId }) }),

  startSocialLogin: (provider: 'google' | 'github') =>
    goRequest('/auth/kiro/start-social', { method: 'POST', body: JSON.stringify({ provider }) }),

  exchangeSocialCode: (data: { sessionId: string; callbackUrl?: string; code?: string }) =>
    goRequest('/auth/kiro/exchange-social', { method: 'POST', body: JSON.stringify(data) }),

  startOpenAIOAuth: () => goRequest('/auth/openai/start', { method: 'POST' }),

  pollOpenAIOAuth: (sessionId: string, callbackUrl?: string) =>
    goRequest('/auth/openai/exchange', { method: 'POST', body: JSON.stringify({ sessionId, callbackUrl }) }),
